# 👋 Hi, I'm Kabin Gamal

🎓 Bachelor's Student | Aspiring Data Analyst / Data Scientist
📍 Kathmandu, Nepal

---

## 🚀 About Me

I am a motivated learner currently pursuing my Bachelor's degree, with a growing focus on **Data Analysis and Data Science**.

I started my journey with **frontend development**, building projects using HTML, CSS, and JavaScript, which helped me develop strong problem-solving and logical thinking skills.

Now, I am actively transitioning into the **data field**, where I work with data to extract insights, build analytical solutions, and continuously improve my understanding of real-world data problems.

I believe in **learning by building**, and I regularly work on projects to strengthen my practical knowledge.

---
## 🛠️ Skills & Tools

### 📊 Skills
* HTML, CSS, JavaScript
* Python (for Data Analysis)
* Pandas (Data Manipulation & Analysis)
* Basic SQL
* Data Cleaning & Transformation

### ⚙️ Tools

* Git & GitHub
* VS Code
* Excel

---

## 📈 Goals

* Build strong projects in Data Analysis & Machine Learning
* Secure a Data Science internship
* Grow towards a career in the **automobile industry**
* Work internationally, with a long-term goal of working in Germany 🇩🇪

---

## 🌱 A Bit More About Me

* 📍 Based in Kathmandu
* ⚽ Enjoy playing football
* ✈️ Love traveling and exploring new places
* 📚 Passionate about continuous learning and self-improvement

---

Feel free to explore my repositories and connect with me!

---
